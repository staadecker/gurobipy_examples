...
def main():
    ...
    for r in regions:
        m.addLConstr(
            gp.LinExpr([(1, build[s]) for s in sites if coverage[s, r] == 1]),
    ...
    m.addLConstr(
        gp.LinExpr([(costs[s], build[s]) for s in sites]),
        gp.GRB.LESS_EQUAL,
        budget,
        name="budget",
    )
    ...
    # Create objective
    m.setObjective(
        gp.LinExpr([(population[r], covered[r]) for r in regions]), gp.GRB.MAXIMIZE
    )
    ...
    res_covered = pd.DataFrame.from_dict(
        {r: covered[r].x for r in regions}, orient="index", columns=["covered"]
    )
    res_covered.index = res_covered.index.rename("region")
    save_data(res_covered, "covered.csv")

    res_build = pd.DataFrame.from_dict(
        {s: build[s].x for s in sites}, orient="index", columns=["build"]
    )
