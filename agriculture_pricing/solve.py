...
def main():
    ...
    # Capacity constraint
    for c in COMPONENTS:
        m.addLConstr(gp.LinExpr([(composition[d, c] / 100, Demand[d]) for d in DAIRIES]), gp.GRB.LESS_EQUAL, capacity[c],
                     name=f'con_capacity_{c}')

    # Price index constraint
    m.addLConstr(
        gp.LinExpr([(last_year_demand[d], Price[d]) for d in DAIRIES]), gp.GRB.LESS_EQUAL, price_index,
        name='con_price_index')

    # Elasticities
    for d in DAIRIES:
        m.addLConstr(
            (Demand[d] - last_year_demand[d]) / last_year_demand[d],
            gp.GRB.EQUAL,
            -elasticity[d, d] * (Price[d] - last_year_price[d]) / last_year_price[d] \
                     + gp.LinExpr([(elasticity[d, d2] / last_year_price[d2], Price[d2]) for d2 in DAIRIES if d != d2]) \
                     - sum(elasticity[d, d2] for d2 in DAIRIES if d != d2), name=f'con_elasticity_{d}')
    ...
    market = pd.DataFrame.from_dict({d: (Demand[d].x, Price[d].x) for d in DAIRIES}, orient='index',
